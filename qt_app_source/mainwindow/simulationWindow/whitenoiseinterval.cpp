#include "whitenoiseinterval.h"
#include <QMessageBox>
#include <mainwindow/mainwindow.h>
WhiteNoiseInterval::WhiteNoiseInterval(MainWindow *m_wind) : BaseSimulionWindow(m_wind)
{
    QLabel *label = new QLabel("Белый шум,  равномерно распределенный в интервале ");
    this->box_layout->addWidget(label);

    label = new QLabel("Введите границу интервала a");
    this->box_layout->addWidget(label);

    this->a = new QLineEdit();
    this->box_layout->addWidget(a);

    label = new QLabel("Введите границу интервала b");
    this->box_layout->addWidget(label);

    this->b = new QLineEdit();
    this->box_layout->addWidget(b);
    ...
    connect(this->simulation_button, &QPushButton::released, this, &WhiteNoiseInterval::simulateSignal);
}

double WhiteNoiseInterval::frand(){
    return (double) rand()/RAND_MAX;
}

void WhiteNoiseInterval::simulateSignal(){
    this->readBaseParametrs();
    if ( this->main_window->main_data_from_file == nullptr ){
        return;
    }

    if ( this->a->text().size() == 0 || this->b->text().size() == 0 ){
    ...
    for ( int i = 1; i <= new_signal->number_of_samples; i++ ){
        this->new_signal->values_of_signal.push_back(
                    this->a->text().toDouble()+(this->b->text().toDouble()-a->text().toDouble())*frand()
                    );
    }
